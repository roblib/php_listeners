...
class IslandoraServiceTest extends PHPUnit_Framework_TestCase {
  ...
  function testAuthorize() {
    $result = $this->islandoraServ->authorize('taverna', 'taverna');
    $this->assertEquals('These microservices are authorized: read,write,allOcr,ocr,hOcr,encodedOcr,jpg,jp2,tn,techmd,scholarPolicy,addImageDimensionsToRels,scholarPdfa,', $result);
    $result = $this->islandoraServ->authorize('taverna', 'testDoNotExist');
    $this->assertEquals('These microservices are authorized: ,', $result);
  }

  function testRead() {
    //currectly authorized
    $result = $this->islandoraServ->authorize('taverna', 'taverna');
    $this->assertEquals('These microservices are authorized: read,write,allOcr,ocr,hOcr,encodedOcr,jpg,jp2,tn,techmd,scholarPolicy,addImageDimensionsToRels,scholarPdfa,', $result);
    $result = $this->islandoraServ->read('islandora:313', 'TEST_TXT', NULL);
    $this->assertEquals('test text', base64_decode($result));

    $result = $this->islandoraServ->read('islandora:313', 'SOMETHING_NOT_EXIST', NULL);
    $this->assertNull($result);

    //not currect authorized
    $result = $this->islandoraServ->authorize('taverna', 'notexist');
    $this->assertEquals('These microservices are authorized: ,', $result);
    $result = $this->islandoraServ->read('islandora:313', 'TEST_TXT', NULL);
    $this->assertNull($result);
  }
