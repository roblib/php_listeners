        ...
        <resource xmlns="http://datacite.org/schema/kernel-3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://datacite.org/schema/kernel-3 http://schema.datacite.org/meta/kernel-3/metadata.xsd">
            ...
            <identifier identifierType="DOI">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:choose>    
                            <xsl:when test="r:Citation/r:InternationalIdentifier/r:ManagingAgency/text() = 'Datacite'">
                                <xsl:value-of select="r:Citation/r:InternationalIdentifier[r:ManagingAgency/text() = 'Datacite']/r:IdentifierContent"/>
                            </xsl:when>
            ...
            <titles>
                <xsl:for-each select="r:Citation/r:Title">
                    <xsl:if test="@translated='false' or not(normalize-space(@translated))">
                       <title>                            
            ...
            <xsl:if test="r:Citation/r:Contributor">
                <contributors>
                    <xsl:for-each select="r:Citation/r:Contributor">
                        <contributor>
                            <xsl:if test="r:ContributorRole">
                                <xsl:attribute name="contributorType"><xsl:value-of select="r:ContributorRole"/></xsl:attribute>                                        
                            </xsl:if>
                            <contributorName>
                                <xsl:call-template name="formatName">
                                    <xsl:with-param name="name" select="."/>
            ...
            <xsl:if test="normalize-space(r:Citation/r:Copyright)">
                <rightsList><rights><xsl:value-of select="r:Citation/r:Copyright"/></rights></rightsList>
            </xsl:if>        
